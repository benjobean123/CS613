...
int main()
{
    ...
    double* myMatrix2[MSize2];
    for(i=0; i<MSize2; i++){
        myMatrix2[i]=new double[MSize2];
    }
    ...
    while(getline(matrix2Read, line)){
        ...
        {
            myMatrix2[i][j] = stod(matrix2Value); //place values in matrix
    ...
    // Multiply matricies
    double* matrixMul[MSize];
    for(i=0; i<MSize; i++){
        matrixMul[i]=new double[MSize];
    }
    //start clock
    clock_t mulTime;
    clock_t mulTimeDif;
